...
public class GameMain extends ApplicationAdapter {
	...
	public void render () {
		...
		if (menu) {
			...
			if (pause) {
				...
				if (whichButton == 2) {
					batch.draw(mm.getArrowAnim(), 440, 150);
				...
				if (whichButton == 0) {
					batch.draw(mm.getArrowAnim(), 440, 350);
					if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE)) {
						if(fullscreen){
	...
	public void renderMenu() {
		...
		if (!helpMenu) {
			...
			if(whichButton == 2) {
				batch.draw(mm.getArrowAnim(), 440, 50);
		...
		if (!helpMenu) {
			...
			if (whichButton == 0) {
				batch.draw(mm.getArrowAnim(), -10, 400);
			...
			if(whichButton == 1) {
				batch.draw(mm.getArrowAnim(), -10, 225);
			...
			if (whichButton == 2) {
				batch.draw(mm.getArrowAnim(), -10, 50);
	...
	public void playerManage() {
		...
		if (RoomManager.getCurrentRoom() instanceof HealingRoom) {
			HealingRoom room = (HealingRoom)(RoomManager.getCurrentRoom());
			if (room.getHealth() > 0) {
				...
				switch (room.getDoors().get(cnt).getLocation()) {
					case "top":
						if (room.getDoors().get(cnt).getNextRoom() instanceof BossRoom) {
							batch.draw(room.getDoors().get(cnt).getBossDoor(), room.getDoors().get(cnt).getxLoc(), room.getDoors().get(cnt).getyLoc() + 10);
						} else {
					...
					case "left":
						if (room.getDoors().get(cnt).getNextRoom() instanceof BossRoom) {
							batch.draw(room.getDoors().get(cnt).getBossDoor(), room.getDoors().get(cnt).getxLoc() - 5, room.getDoors().get(cnt).getyLoc());
						} else {
